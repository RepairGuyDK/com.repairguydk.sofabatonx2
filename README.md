...
1. **Run an MQTT broker.** Easiest: install the "MQTT Broker" app by
   *Menno van Grinsven* (https://homey.app/a/nl.scanno.mqttbroker/) on Homey
   ...
   connections"** and **"Disable user authentication"** (or set a username &
   password and use the same ones on the device), then press **"Start Broker"**
   and confirm it shows **"Broker is Running"**.
